...
func fakeSocialProfile() *article.SocialProfile {
	return &article.SocialProfile{
		ID:       gofakeit.UUID(),
...
func TestNewSocialProfilesStrict(t *testing.T) {
	t.Run("valid social profiles", func(t *testing.T) {
		profile1 := fakeSocialProfile()
		profile2 := fakeSocialProfile()
		profiles, err := article.NewSocialProfilesStrict(profile1, profile2)
		assert.NoError(t, err)
		assert.Equal(t, 2, profiles.Len())
	})

	t.Run("invalid social profiles", func(t *testing.T) {
		profile1 := fakeSocialProfile()
		profile2 := &article.SocialProfile{ID: "", URL: ""}
		profiles, err := article.NewSocialProfilesStrict(profile1, profile2)
		assert.Error(t, err)
...
func TestNewSocialProfiles(t *testing.T) {
	t.Run("mixed valid and invalid social profiles", func(t *testing.T) {
		profile1 := fakeSocialProfile()
		profile2 := &article.SocialProfile{ID: "", URL: ""}
		profile3 := fakeSocialProfile()
		profiles := article.NewSocialProfiles(profile1, profile2, profile3)
		assert.Equal(t, 2, profiles.Len())
	})
}
...
func TestSocialProfiles_Get(t *testing.T) {
	profile1 := fakeSocialProfile()
	profile2 := fakeSocialProfile()
	profiles := article.NewSocialProfiles(profile1, profile2)
...
func TestSocialProfiles_Add(t *testing.T) {
	profiles := article.NewSocialProfiles()
	profile1 := fakeSocialProfile()
	profile2 := &article.SocialProfile{ID: "", URL: ""}
	profile3 := fakeSocialProfile()
...
func TestSocialProfiles_Filter(t *testing.T) {
	...
	t.Run("filter with always true function", func(t *testing.T) {
		filtered := profiles.Filter(func(profile *article.SocialProfile) bool {
			return true
		})
...
func TestSocialProfiles_Normalize(t *testing.T) {
	...
	invalid := &article.SocialProfile{ID: gofakeit.UUID(), URL: " sdf "}

	profiles := article.NewSocialProfiles(valid, invalid)
	profiles.Normalize()
